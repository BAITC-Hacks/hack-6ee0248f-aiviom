...
The Atlas release is separate from the hackathon submission. Do not activate it before 2026-09-23 18:00 Asia/Almaty (13:00 UTC). Preserve submission tag and SHA above; never rewrite them. Existing production is image `career-quest:1c397c1f0c10652707e34d9d0f3d4776eda0e97b`, Compose directory `/home/alex/career-quest/releases/1c397c1` (read-only rechecked 2026-09-23 11:01 UTC).
...
Any scheduled continuation must recheck actual time, current production SHA and Git main before activation; stop on unexpected concurrent changes. Main can fast-forward to the checked release only after the submission freeze ends. The captain submits the organizer form; deploying Atlas does not alter that submission.

...
Репозиторий private по правилам организатора: нужен выданный организатором доступ GitHub. Никакой персональный OpenAI аккаунт для локальной проверки не требуется, когда доступен командный judge gateway. Командный gateway развёрнут и проверен: чистый clone без `.env` и личного ключа получил `live_ai` на новом профиле за **6,24 с**. При недоступности сети включается явно обозначенный расчётный режим.
...
`npm test` и `npm run verify:source` не вызывают платный API. `npx tsx scripts/live-smoke.ts` — отдельный платный smoke только при заданном серверном ключе. Проверки покрывают review cutoff/proxy, caps, EV_036, prerequisites, preview, import, RBAC, изоляцию, идемпотентность и AI output. **39/39 тестов**, сборка и чистый запуск прошли; перезапуск сохраняет импорт, навыки и XP. Карта/статус: [ACCEPTANCE](docs/ACCEPTANCE.md).
...
Конфигурация Docker: `compose.yml`, контейнер нового проекта слушает только `127.0.0.1:3500` сервера. [Изолированный deployment](docs/DEPLOY.md). Рабочий demo URL: **https://career.aiviom.ai**. HTTPS, health и live AI проверены. IP: **34.165.120.4**.
